...
class DBReaderMethod(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        reader = DBReader("IPQualityScore-IP-Reputation-Database-IPv4.ipqs")
        await reader.init()
        self.db_ipv4 = await reader.Fetch("8.8.0.0")
        await reader.close()

    async def test_IPv4IsProxy(self):
        self.assertEqual(self.db_ipv4.IsProxy(), False)

    def test_IPv4IsVPN(self):
        self.assertEqual(self.db_ipv4.IsVPN(), False)
    ...
    def test_FraudScore(self):
        self.assertEqual(self.db_ipv4.FraudScore(), 0)
    ...
    def test_ISP(self):
        self.assertEqual(self.db_ipv4.ISP(), "Lumen")
    ...
    def test_ASN(self):
        self.assertEqual(self.db_ipv4.ASN(), 3356)
